...
## Last updated: 2026-06-20 (v1.5.9)
...
4. **OR Staffing Budget Calculator** ✅ complete (v1.5.3–v1.5.5 history below; v1.5.6 introduced the two-file PDF-driven shape; v1.5.7 added display-only fixes — stale-report banner, today's-row mixed-data note, weekend variance suppression; v1.5.8 fixed the join so weekend days with zero scheduled OR cases still appear; **v1.5.9** is the current shape, reworking the join to a date-range scope (bounded by the productivity report's own date coverage) and adding a per-row past/today/future timeline color bar + today-row highlight, plus a fixed-height scrollable table with a working sticky header. Takes **two files** — the OR Schedule XLSX + the Productivity Report **PDF** — joins them on date, and shows a 9-column per-day table comparing budgeted vs. productivity-report hours/FTE for WBVC OR. Decoupled from the shared `_runAllAudits`. See Key Decisions.)
...
- Most recent release: `dev` merged into `main` at v1.4.29 (2026-06-17), fast-forwarded — tomboonern.com is current through v1.4.29. v1.5.0 through v1.5.9 are on `dev` (tomboone.io) only, not yet released to `main`.
...
- Staffing range-based join + timeline highlighting + sticky-header fix (v1.5.9): CPT/Equipment/Room Rules untouched; all changes confined to `auditStaffing`, `buildStaffingRow`, `renderStaffingResults`, `parseProductivityPdf` (diagnostics removal only), index.html, and styles.css.
  ...
  - **(3) Sticky header fix.** The table's wrapper div gained a second class: `class="table-wrap staffing-table-wrap"` (index.html). New CSS: `.staffing-table-wrap { overflow-y: auto; max-height: 520px; }` — the pre-existing `.staffing-table thead th { position: sticky; top: 0; }` (from v1.5.6) only works within a scrolling container, which `.table-wrap` alone didn't provide; now the table body scrolls within its own 520px-tall box and the header sticks to the top of that box.
