...
## Last updated: 2026-06-01 (v1.3.69)
...
- Violations table Case # column: bold; two separate listeners — `violCaseCell` click copies to clipboard (no stopPropagation, bubbles up); `tr` click calls `jumpToCase`. Document `pointerdown` handler excludes `#roomRulesViolationsTable` so it never prematurely closes the sidebar before the row click fires.
