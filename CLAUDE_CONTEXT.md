...
## Last updated: 2026-06-20 (v1.5.7)
...
4. **OR Staffing Budget Calculator** ✅ complete (v1.5.3–v1.5.5 history below; v1.5.6 introduced the two-file PDF-driven shape; **v1.5.7** is the current shape, adding display-only fixes — stale-report banner, today's-row mixed-data note, weekend variance suppression. Takes **two files** — the OR Schedule XLSX + the Productivity Report **PDF** — joins them on date, and shows a 9-column per-day table comparing budgeted vs. productivity-report hours/FTE for WBVC OR. Decoupled from the shared `_runAllAudits`. See Key Decisions.)
...
- Most recent release: `dev` merged into `main` at v1.4.29 (2026-06-17), fast-forwarded — tomboonern.com is current through v1.4.29. v1.5.0 through v1.5.7 are on `dev` (tomboone.io) only, not yet released to `main`.
