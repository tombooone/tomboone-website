...
- CPT code links: `makeCptLink(code, child)` helper wraps any element in `<a href="https://www.aapc.com/codes/cpt-codes/${code}" target="_blank" rel="noopener noreferrer">` with underline + pointer cursor. Used in: Table 1 (`appendCodeText` — wraps `<strong>` elements), Table 2 (`missingCodesTd` — wraps amber marks), Table 3 (wraps red marks in both unrecognized and errorMessages rows).
