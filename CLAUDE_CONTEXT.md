...
## Last updated: 2026-06-09 (v1.4.8)
...
- Equipment audit results table: columns are Date, Case #, Location, Surgeon (optional), Special Needs, Explanation (6 cols). Detail pane colSpan=6. Surgeon column uses `parseSurgeonLastName()`; surgeonId uses `extractSurgeonId()`. Date uses `parseDateCell()`. KEYWORD_ALIASES maps keyword → alias strings checked first in `containsEquipmentTerm()` (e.g. PTeye→parathyroid). KEYWORD_DISPLAY_NAMES maps keyword → display name used in explanation string (e.g. Neoprobe→TruNode). SURGEON_EQUIPMENT_PREFS keyed by surgeon ID string with optional `ultrasound`/`microscope` fields; shown in "Surgeon Preference" section of detail pane, filtered by flagged keyword. Table 3 (Invalid CPT) uses class `cpt-invalid-table` with ~8% Date, ~12% Location, ~10% Case #, ~70% Issue column widths. Equipment List in detail pane renders items exactly as they appear in the Epic export (no W-prefix stripping — `.replace(/^W\s+/, "")` was removed in v1.4.8).
