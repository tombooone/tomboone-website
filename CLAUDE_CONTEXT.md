...
## Last updated: 2026-08-19 (v1.7.9 — live on dev, not yet on main)
...
- Current version: **v1.7.9** (live on dev only, not yet on main)
...
**`.accordion-description` shared class (v1.7.9, `styles.css`):** both this Table 1 note and the CME accordion's equivalent note (originally added v1.7.2, worded independently of this one) were duplicating the exact same inline `style.cssText` in `app.js` rather than sharing a class. v1.7.9 extracted `.accordion-description` (`color: var(--ink); font-weight: 700; font-size: 0.82rem; margin: 0 0 10px 12px;`) and pointed both `note.className` assignments at it instead of the inline string — bumped from a muted, normal-weight, flush-left `<p>` to bold/near-black (`var(--ink)`, the site's standard dark text color, not `var(--muted)`) with a 12px left margin matching the table cells' own `padding: 11px 12px` horizontal inset, so the note's text lines up with the column-header text below it rather than floating flush against the container edge. Font-family/size intentionally unchanged. Verified via computed-style + text-node `Range` measurement in both tools: `fontWeight: 700`, `color: rgb(23, 50, 77)` (`--ink`), text left-edge pixel-identical to the table's own header-cell text left-edge, in both CPT Table 1 and CME. Any future third instance of this "persistent accordion description" pattern should reuse `.accordion-description` rather than a new inline style.
