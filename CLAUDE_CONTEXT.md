...
1. **CPT Audit Tool** ✅ complete (v1.6.2 reverted the missing/not-on-order comparison to presence-based — see Data Schema section note below; otherwise do not touch)
...
- Known Problem CPTs: `KNOWN_PROBLEM_CPTS` is a hardcoded array in `rules-data.js` (not app.js) — entries have `{ code, description, dateAdded, ticket }` fields. As of v1.6.7 the array contains 5 entries (all dateAdded: "2026-06-17", ticket: "Pending"): J7296 (Kyleena IUD), J7301 (Skyla IUD), Q0091 (Pap smear), Q9967 (contrast material), 44394 (colonoscopy through stoma with snare). **45386** (colonoscopy with transendoscopic balloon dilation) was removed in v1.6.6; **J7297** (Liletta IUD) and **J7298** (Mirena IUD) were removed in v1.6.7 — all three will once again surface normally in the Missing/Not on Order columns when they appear, no longer suppressed. All codes remaining in the array are excluded from `missingCodes`, `notOnOrderCodes`, `invalidCodes`, and `inpatientMatches` until removed from this array. A "View known problem CPTs" ghost button in the CPT audit instructions panel navigates to `knownProblemCptsView`, which renders a 4-column table (Code, Description, Date Added, Ticket) or "No known problem CPTs on file." only when the array is empty. Table 2 Missing cells show each code as `<mark>` + "Click to report CPT not in Epic" button; clicking opens a mailto to Thomas.Boone@SutterHealth.org with subject "CPT Not in Epic" and body "CPT CODE: XXXXX".
...
- Staffing scope + inline per-row inputs (v1.5.5): the OR Staffing Budget Calculator was reworked (CPT/Equipment/Room Rules untouched). **(1) Future-days filter:** `auditStaffing` now only counts cases whose `parseDateCell(...).sort` is **strictly greater than today's local midnight** (`const todayMidnight = new Date(); todayMidnight.setHours(0,0,0,0); … dateValue.sort > todayMidnight.getTime()`), so a partial current day / past dates no longer leak in (this fixed an anomalous low-minute first row). This date filter is **staffing-only** — CPT/Equipment/Room Rules do not filter by date. **(2) WBVC OR scope:** `staffingColumns` gained an **optional** `room` column (`accepted: ["room","room (as scheduled)","or room"]`); `auditStaffing` builds the same `roomPrefixRegex` as `auditRoomRules` from `CAMPUS_CONFIG.WBVC.roomPrefix` and includes only rows whose Room matches "WBVC OR" — applied only when a Room column is present (`hasRoom = indexes.room !== -1`; if absent it falls back to all rooms). A "Currently configured for CPMC Van Ness (WBVC OR) only" note was added under the page heading (same style as the OR Schedule Audit note). **(3) Summary tiles removed:** the three `.summary-grid` metric tiles (Days shown / Total OR minutes / Total staff hours) and their IDs (`staffingDayCount`/`staffingTotalMinutes`/`staffingTotalHours`) are gone; `auditStaffing` no longer returns `totalMinutes`/`totalStaffHours` (just `{ totalRows, days }`). **(4) Comparison panel removed:** `#staffingSchedulePanel`, the day dropdown, `selectStaffingDay`, `updateStaffingComparison`, module state `_staffingResult`/`_staffingSelectedIndex`, and the init-time dropdown/input listeners were all deleted. **(5) Redesigned per-day table** (`.staffing-table`, 8 columns): **Date | OR Minutes** (whole) **| OR Hours** (min÷60, 1 dp) **| Staff Hours Budgeted** (min×0.103, 1 dp) **| FTEs Budgeted** (staffHours÷40, 1 dp) **| FTEs Scheduled | FTE Variance | Hours Variance**. The **FTEs Scheduled** cell is a self-contained widget (`buildStaffingRow(d)`): three small number inputs labeled 8/10/12 (`.staffing-sched-inputs`, default 0) above a live total line (`.staffing-sched-total`) reading `"X.X FTEs / XX.X hrs"` — **Scheduled FTEs** = 8hr×0.2 + 10hr×0.25 + 12hr×0.3 (`STAFFING_CONFIG.shiftFte`), **Scheduled Hours** = 8hr×8 + 10hr×10 + 12hr×12 (so "Hours Scheduled" is shown inside this cell, NOT a separate column). **FTE Variance** = FTEs Budgeted − Scheduled FTEs; **Hours Variance** = Staff Hours Budgeted − Scheduled Hours; both signed to 1 dp via `setStaffingVariance(td, value)` (positive/under = green `.staffing-var-under` #1a7f4b, negative/over = red `.staffing-var-over` #b42318, |v|<0.05 = neutral green "0.0", never "-0.0"). Each row recomputes only its own total + two variance cells on every `input` event (independent closures); inputs `stopPropagation` on click so they never bubble to a row-level handler. Empty state colSpan is 8 ("No upcoming WBVC OR cases…"). Item 6 of the task (verify both `toggleAffordance.style.cssText` read `"margin-top:4px;"` with no `display:block`) was confirmed already-correct from v1.5.2 — no change. Verified in headless Chrome (today 2026-06-19, sample with future 6/22 + 6/23 WBVC rows plus a WBMB row, a today row, a past row, and a no-proj-time row all correctly excluded): two day rows render (6/22 = 180 min/3.0 hrs/18.5 staff hrs/0.5 FTE; 6/23 = 270 min/4.5 hrs/27.8/0.7); inputs live-update (1×8+1×10 → "0.5 FTEs / 18.0 hrs", FTE var 0.0 green, Hours var +0.5 green; 5×12 → "1.5 FTEs / 60.0 hrs", FTE var −0.8 red, Hours var −32.2 red); rows independent; summary tiles + schedule panel absent; all four tools run without error. **NOTE (superseded by v1.5.6):** the staffing-only future-days filter, the inline 8/10/12 inputs, the `staffing-sched-*` CSS, and the 8-column layout described here were all replaced in v1.5.6 (scheduled hours now come from a Productivity PDF, not manual inputs).
- Prospective date filter + PDF-driven staffing overhaul (v1.5.6):
  - **(1) Prospective date filter on the OTHER three tools.** `auditRows` (CPT), `auditEquipmentRows`, and `auditRoomRules` each now compute `const tomorrowMidnight = new Date(); tomorrowMidnight.setHours(24,0,0,0); const tomorrowMs = tomorrowMidnight.getTime();` and apply `.filter((row) => parseDateCell(cell(row, indexes.date)).sort >= tomorrowMs)` to `dataRows` right after `populatedRows.slice(headerRowIndex+1)` (for Room Rules, ANDed into the existing `roomPrefixRegex` filter). So all three now show **upcoming days only** (dates ≥ tomorrow's local midnight; unparseable dates → `MAX_SAFE_INTEGER` so they're kept). This is the only change to those three functions — matching/audit logic is untouched. `totalRows` (= `dataRows.length`) reflects the post-filter count. **`auditStaffing` does NOT apply this filter** (it shows all dates, limited instead by the PDF join). Verified: a fixture with 1 past + 9 future rows → CPT/Equipment "Reviewed 9", Room Rules "Reviewed 8" (8 future WBVC OR; the WBMB row and the past row excluded).
  - **(2) Staffing is now a two-file, independent tool.** It was removed from `_runAllAudits` and `_showCachedResult` (it needs a second file). New `wireStaffingTool()` (replaces the old `wireAuditTool({toolKey:"staffing"})`) owns two inputs: `#staffingFileInput` (XLSX OR schedule) and the new `#staffingPdfInput` (PDF). **Run is enabled only when BOTH files are selected**; on Run it `readXlsxRows(xlsxFile, staffingColumns)`, `parseProductivityPdf(pdfFile)`, then `auditStaffing(rows, prodHoursMap)` → `renderStaffingResults`. Returns `{ reset, showFromShared }` (showFromShared is a no-op; staffing no longer reads `sharedAuditData`). The shared Clear still calls `_staffingTool.reset()`.
  - **(3) PDF parsing.** `parseProductivityPdf(file)` uses **PDF.js** (global `pdfjsLib`, loaded from cdnjs — see External dependency note). Sets `GlobalWorkerOptions.workerSrc` to the matching cdnjs worker, reads all pages' text, and runs `/(\d{2}\/\d{2}\/\d{4})\s+[\d.]+\s+([\d.]+)\s+[\d.]+\s+[\d.]+/g` over the joined text → `Map<sortDate, prodHours>` keyed by `parseDateCell(MM/DD/YYYY).sort`. Columns are Date, Weighted Census, **Prod. Hours** (group 2 = the value used), Budget, Target. **Prod. Hours of 0.00 is skipped** (treated as "no data").
  - **(4) `auditStaffing(rows, prodHoursMap)`** — WBVC OR scope (same optional `room` column + `roomPrefixRegex`), **no date filter**, sums Proj End − Proj Start per day, then for each day with minutes > 0 looks up `prodHoursMap.get(sortDate)` and **only includes the day if a non-zero prod-hours entry exists** (days with OR minutes but no productivity match are silently dropped). Per-day object: `{ display, sortDate, minutes, orHours (min÷60), hrsBud (min×0.103), fteBud (hrsBud÷40), prodHours, fteSched (prodHours÷40), fteVar (fteBud−fteSched), hrsVar (hrsBud−prodHours) }`. **The XLSX↔PDF date join works because `parseDateCell` maps both Excel-serial and MM/DD/YYYY strings to LOCAL midnight** (`excelSerialToDate` returns `new Date(y,m,d)`; `Date.parse("MM/DD/YYYY")` is local) — so ISO `YYYY-MM-DD` test fixtures would NOT join (use US-format/serial dates).
  - **(5) Table redesign (9 columns, `table-layout: fixed`, each col `calc(100%/9)`):** **Date | OR Min | OR Hrs | Hrs Bud | FTE Bud | Hours | FTE | FTE Var | Hrs Var**. "Hours"/"FTE" are the productivity-report (scheduled) values; FTE Var / Hrs Var use `setStaffingVariance` (green ≥ −0.05 / red < −0.05 / neutral "0.0"). The inline-input widget is gone. Date cell shows weekday name + `M/D/YYYY` on two lines (`new Date(sortDate)`, `.staffing-date-sub`). **Weekend rows** (`getDay()` 0 or 6) get `.staffing-weekend` (bg `#f0f4f8`); **Saturday rows** also get `.staffing-sat-border` (`border-bottom: 2px solid #94a3b8`). **Sticky header:** `.staffing-table thead th { position: sticky; top: 0; z-index: 2; background:#eaf5f8; }`. Empty state colSpan 9.
  - **(6) FTE label cleanup:** every user-facing "FTEs" in the staffing view + home tile → "FTE" (headers are "FTE Bud" / "FTE" / "FTE Var").
  - **(7) Instructions:** "Staff hours budget is calculated by multiplying OR minutes by 0.103. One FTE is 40 hours per week." Two-file steps; WBVC note retained under the heading.
  - **(8) Accordion arrow:** both `toggleAffordance.style.cssText` confirmed `"margin-top:4px;"` (no change).
  - Verified end-to-end in headless Chrome (today 2026-06-19) with a real generated PDF + MM/DD/YYYY HTML fixture: both files load → Run enables → PDF parsed from CDN → "4 days matched"; the 4 rows (6/22, 6/23, 6/27 Sat, 6/28 Sun) show correct joined values (e.g. 6/22 = 180 min / 3.0 / 18.5 / 0.5 / 425.1 / 10.6 / −10.2 / −406.5), sticky header `position:sticky`, weekday two-line dates, weekend shading on Sat+Sun with the thick border only under Saturday, and red over-budget variances; 6/24 (PDF 0.00) and 6/10 (no PDF entry) correctly excluded. Variance colors re-checked: +2.3→green, −0.01→green "0.0", −2.3→red.
- Staffing tool display fixes (v1.5.7): CPT/Equipment/Room Rules, PDF parsing, the join logic, and FTE/hours math are all untouched — display-only changes in `renderStaffingResults`/`buildStaffingRow` plus a small addition to `parseProductivityPdf`'s return shape.
  - **(1) Stale-report banner.** `parseProductivityPdf` now also extracts a single report date from the PDF footer via `/Report Date\s*:\s*(\d{2}\/\d{2}\/\d{4})/i` and returns `{ map, reportDate }` (was: bare `map`) — `reportDate` is `parseDateCell(...)` of the captured string, or `null` if the footer pattern isn't found. `wireStaffingTool`'s run handler destructures `{ map: prodHoursMap, reportDate }` and passes `reportDate` through to `renderStaffingResults(result, reportDate)`. `renderStaffingResults` compares `reportDate.sort` to today's local midnight; if they differ (and `reportDate` is non-null/finite) it unhides `#staffingStaleBanner` (a `<p>` in index.html inside `#staffingResultsPanel`, styled `.staffing-stale-banner` in styles.css — amber `var(--warn-soft)` background, `var(--warn)` text, 4px `var(--accent)` left border, matching the existing `.error-row`/Tier-3 amber palette) with text `"This productivity report is from [MM/DD/YYYY] — data may not reflect today's actuals."`; otherwise (dates match, or no report date was found at all) the banner stays `hidden`.
  - **(2) Today's-row mixed-data note.** In `buildStaffingRow(d)`, if `d.sortDate` equals today's local midnight, a `<span class="staffing-today-note">⚠ Mix of actual & scheduled OR min</span>` is appended inside the date `<td>` below the weekday/date lines. `.staffing-today-note` (styles.css) is `display:block; font-size:0.75rem; color:var(--muted); margin-top:2px`. No other row gets this note.
  - **(3) Weekend variance suppression.** In `buildStaffingRow(d)`, for rows where `dow` (already computed for `.staffing-weekend`) is 0 or 6, the FTE Var / Hrs Var `<td>`s are set via a local `signed()` helper (same ±1-decimal/neutral-"0.0" formatting as `setStaffingVariance` but with **no class added** — plain ink-colored text) instead of calling `setStaffingVariance`. Weekday rows are unchanged (still call `setStaffingVariance`, still get `.staffing-var-under`/`.staffing-var-over`). A new muted `<p>` was added below the table in index.html (inline-styled, matching the existing per-panel instructional-note convention — no new CSS class): "Weekend variance reflects call coverage staffing against near-zero scheduled OR minutes and is not a meaningful budget indicator."
  - **(4) 6/27 & 6/28 investigation — diagnostic logging added, no logic changed.** Added two `console.log` blocks (clearly temporary/investigative, not gated behind a flag — should be removed once root-caused against real data): one in `parseProductivityPdf` right after building `map`, logging `prodHoursMap.get(...)` for 6/27/2026 and 6/28/2026 (confirms whether the PDF regex matched those rows); one in `auditStaffing` right after `byDate` is built, logging summed OR minutes for the same two dates from the XLSX side (confirms whether the join's other input is present). Verified in headless Chrome with synthetic fixtures (real `pdfjsLib`-parsed PDF + HTML-table "XLSX") that both dates flow through correctly end-to-end when both inputs contain non-zero data for them — so the prior missing-rows symptom is environment/data-dependent (a real Epic export + PDF would need to be run through this logging to see which side is actually empty for 6/27/6/28); the join logic itself (`auditStaffing`'s `if (!(prodHours > 0)) return;`) was confirmed NOT to be the bug in isolation and was left unchanged per instructions.
  - Verified in headless Chrome via Playwright with a generated PDF (reportlab) containing a `Report Date : 06/19/2026` footer and 5 WBVC OR rows (6/20 Sat = "today", 6/22, 6/23, 6/27 Sat, 6/28 Sun) joined against a matching HTML-table XLSX fixture: banner shown with correct stale-date text; 6/20 row showed the mixed-data note (no other row did); 6/20/6/27/6/28 variance cells rendered with no color class (computed color = page ink, not green/red) while 6/22/6/23 kept green `.staffing-var-under`; weekend note text present below table; console logged correct PDF-hours (75, 66) and OR-minutes (360, 330) for 6/27/6/28. Re-tested with report date = today → banner correctly hidden; re-tested with a PDF lacking any "Report Date" line → `reportDate` is `null`, banner stays hidden, no errors.
- Weekend zero-OR-minutes join fix (v1.5.8): root cause of the 6/27/6/28 missing-rows symptom from v1.5.7's investigation confirmed — weekends have **no scheduled cases**, so they never created a `byDate` entry at all (the per-case loop in `auditStaffing` only writes an entry when `orMinutes > 0`), and the join then required `d.minutes > 0` before even checking `prodMap`, so a weekend day with real call-coverage Prod. Hours but zero scheduled OR cases was silently dropped before the productivity lookup ran. **Fix, in `auditStaffing` only:** after building `byDate` from the per-case loop (unchanged), a new `candidateDates` map is seeded with `new Map(byDate)`, then every `sortDate` key in `prodMap` that ISN'T already in `byDate` is added as a zero-minutes placeholder entry (`{ display, sortDate, minutes: 0 }`, with `display` derived directly from `new Date(sortDate)` since `parseDateCell` only accepts spreadsheet-cell-shaped input, not a raw timestamp). The join loop now iterates `candidateDates` (not `byDate`) and computes `const dow = new Date(d.sortDate).getDay(); const isWeekend = dow === 0 || dow === 6;` per day; the inclusion check is `if (!isWeekend && !(d.minutes > 0)) return;` (weekday gate unchanged) followed by the existing `if (!(prodHours > 0)) return;` (unchanged for both weekday and weekend — a non-zero Prod. Hours entry is still required either way). Weekend days that clear this end up with `minutes: 0` → `orHours`/`hrsBud`/`fteBud` all compute to 0 naturally from the existing formulas (no special-casing needed). The v1.5.7 weekend variance-color suppression in `buildStaffingRow` was NOT touched and still applies to these rows. Verified in headless Chrome via Playwright: an XLSX fixture with **zero scheduled cases** on 6/27 (Sat) and 6/28 (Sun) joined against a PDF with non-zero Prod. Hours (75.0 / 66.0) for those dates → both rows now appear (OR Min 0, OR Hrs 0.0, Hrs Bud 0.0, FTE Bud 0.0, Hours 75.0/66.0, FTE 1.9/1.6, FTE Var/Hrs Var unstyled plain text per the v1.5.7 weekend rule); a weekday (Wednesday 6/24) with zero OR minutes and a non-zero PDF entry was confirmed still excluded (weekday gate unchanged — "0 days matched" empty state shown), confirming the fix is weekend-only and doesn't loosen the weekday join. **NOTE (superseded by v1.5.9):** the `isWeekend`-branching inclusion check and the `new Map(byDate)`-seeded `candidateDates` approach described here were replaced in v1.5.9 by a date-range-scoped join — see next bullet.
- Staffing range-based join + timeline highlighting + sticky-header fix (v1.5.9): CPT/Equipment/Room Rules untouched; all changes confined to `auditStaffing`, `buildStaffingRow`, `renderStaffingResults`, `parseProductivityPdf` (diagnostics removal only), index.html, and styles.css.
  - **(1) Join reworked to a date-range scope, replacing the v1.5.8 `isWeekend` branch.** In `auditStaffing`, after `byDate` is built (unchanged), the code now scans `prodMap` once for `minDate`/`maxDate` — the earliest and latest `sortDate` with a non-zero Prod. Hours entry. `candidateDates` is built from scratch (not seeded from `byDate` via `new Map(byDate)` as in v1.5.8): first, `byDate` entries whose `sortDate` falls within `[minDate, maxDate]` are copied in; then any `prodMap` entry with `prodHours > 0` not already present is added as a zero-minutes placeholder (`{ display, sortDate, minutes: 0 }`, `display` derived from `new Date(sortDate)`). If `minDate` is `null` (prodMap has no non-zero entries at all), `candidateDates` stays empty. The final `forEach`'s inclusion check is now `const prodHours = prodMap.get(d.sortDate) || 0; if (!(d.minutes > 0) && !(prodHours > 0)) return;` — no `isWeekend`/`dow` check at all; a day is kept if it has OR minutes OR productivity hours (within the report's date range), full stop. The `|| 0` default on `prodHours` prevents `NaN` from `undefined / 40` etc. when a day has OR minutes but no productivity entry (can now happen for weekday rows just outside the join's old assumptions). Net effect vs. v1.5.8: weekend-vs-weekday is no longer a special case in the join itself — it only still matters for variance-color suppression (unchanged, in `buildStaffingRow`) and for the row's weekend shading classes (unchanged). The two `[staffing]` diagnostic `console.log` blocks added in v1.5.7/v1.5.8 (for investigating the original 6/27/6/28 symptom, one in `parseProductivityPdf`, one in `auditStaffing`) were removed now that the root cause is fixed.
  - **(2) Per-row timeline color bar + today highlight.** In `buildStaffingRow`, `todayMs`/`todayMidnight`, `isToday` (`d.sortDate === todayMidnight`), and `isPast` (`d.sortDate < todayMidnight`) are computed up front. `isToday` adds `.staffing-today-row` to the `<tr>` (styles.css: `background:#dbeafe`, declared after `.staffing-weekend` so today's blue wins over weekend shading on a row that's both, e.g. a Saturday that is today). `timelineColor` is `#10b981` (past/green) / `#3b82f6` (today/blue) / `#f59e0b` (future/amber), applied as `dateTd.style.borderLeft = "4px solid " + timelineColor` directly on the date `<td>` (inline style, not a class) — required because `border-collapse` on the table makes a border set on the `<tr>` itself not render; per-cell is the only way to get a visible left rail. The today note span text changed from "⚠ Mix of actual & scheduled OR min" to **"⚠ Today: includes actual and scheduled cases"** (same `.staffing-today-note` class/styling, unchanged).
  - **(3) Sticky header fix.** The table's wrapper div gained a second class: `class="table-wrap staffing-table-wrap"` (index.html). New CSS: `.staffing-table-wrap { overflow-y: auto; max-height: 520px; }` — the pre-existing `.staffing-table thead th { position: sticky; top: 0; }` (from v1.5.6) only works within a scrolling container, which `.table-wrap` alone didn't provide; now the table body scrolls within its own 520px-tall box and the header sticks to the top of that box. **NOTE (superseded by v1.5.10):** this created a height-capped mini scroll-box rather than a header that pins to the browser viewport as the page itself scrolls — not the intended behavior. See the v1.5.10 bullet below for the corrected approach.
  - **(4) Copy updates.** Stale-report banner text (`renderStaffingResults`) changed to: "This productivity report was generated on [date]. Data shown for dates on or before [date] reflects actual hours worked; later dates reflect scheduled hours." (was: "...is from [date] — data may not reflect today's actuals."). Weekend footer note (index.html, inline-styled `<p>` below the table) changed to: "Weekend staffing reflects standing call coverage; variance is expected and is not a budget concern." (was: "...not a meaningful budget indicator.").
  - Verified in headless Chrome via Playwright: a PDF with Report Date 06/19/2026 (today 2026-06-20, stale) + an XLSX with **zero scheduled cases** on 6/27 (Sat) and 6/28 (Sun) → both weekend rows still appear via the new range-based join (OR Min 0 / OR Hrs 0.0 / Hrs Bud 0.0 / FTE Bud 0.0, Hours/FTE from the PDF, unstyled plain variance text per the existing weekend suppression); the stale banner showed the new two-sentence copy; 6/20 ("today" in this fixture) got `.staffing-today-row` + blue `rgb(59,130,246)` left border + the new note text, while 6/22–6/28 (future) got amber `rgb(245,158,11)`; re-run with a 3-day fixture spanning a past/today/future date (6/18 past, 6/20 today, 6/22 future, report date = today so banner correctly hidden) confirmed past rows get green `rgb(16,185,129)`; `.staffing-table-wrap` computed style confirmed `overflow-y:auto; max-height:520px`; console had zero `[staffing]`-prefixed diagnostic lines (the removed blocks); weekend footer note text confirmed updated.
- Sticky header fix v2 — viewport-level, not container-scroll (v1.5.10): no staffing logic touched — index.html and styles.css only, replacing the v1.5.9 height-capped scroll-box approach. The staffing table's wrapper div is `class="table-wrap staffing-results-wrap"` (was `table-wrap staffing-table-wrap`); `.staffing-table-wrap` (with its `overflow-y:auto; max-height:520px`) was removed entirely. **Root cause of why a plain `.table-wrap` alone breaks `position: sticky`:** `.table-wrap` sets `overflow-x: auto` (styles.css ~line 552) — per the CSS Overflow spec, when one axis is a non-`visible` value and the other isn't explicitly set to the same, the `visible` axis computes to `auto` too (browsers don't support "scroll one axis, not the other" as `visible`+`auto`), so `.table-wrap` is *always* a scroll container on both axes even though only `overflow-x` is written — and any ancestor that's a scroll container becomes the nearest "containing block" for `position: sticky`, capping it to that element's own box instead of the viewport. **Fix:** `.staffing-results-wrap { overflow: visible; }` overrides `.table-wrap`'s `overflow-x: auto` for the staffing table specifically. This rule had to be placed in styles.css **immediately after** the `.table-wrap` rule (not before, where it was first tried) — both selectors are single classes (equal specificity), so the later rule wins; placed before `.table-wrap`, the cascade let `.table-wrap`'s `overflow-x: auto` win and the same auto-resolution problem recurred. (The task's originally-specified `overflow-x: auto; overflow-y: visible;` for `.staffing-results-wrap` was tried first and rejected — same auto-resolution issue applies to a same-element mixed-axis rule, so `overflow-y` still computed to `auto` and sticky stayed broken; confirmed via computed-style inspection in Playwright before settling on plain `overflow: visible`.) `.staffing-table thead th` (`position: sticky; top: 0; z-index: 2; background: #eaf5f8;`) is unchanged — it now works because nothing between it and the viewport is a scroll container. No `max-height` anywhere in the staffing view; the table can grow as tall as its content and the header pins to the top of the browser viewport while the page scrolls past it. Verified in headless Chrome via Playwright with a 24-row fixture (enough to exceed one screen): confirmed `.staffing-results-wrap` computed `overflow-y: visible` (not `auto`); after `window.scrollBy(0, 600)`, the thead `<th>`'s `getBoundingClientRect().top` was exactly `0` (pinned to the viewport edge) while the wrapper's own `scrollTop` stayed `0` (no inner scrollbox was created) — screenshot confirmed the header bar sitting flush against the browser viewport top with table rows scrolling underneath it, not a contained mini scroll-box.
