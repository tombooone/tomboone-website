...
## Last updated: 2026-08-19 (v1.7.4 — live on both dev and main)
...
4. **Gender Reassignment Surgery CME Approval Audit** ✅ complete, **added v1.7.0 (2026-08-18), released to production at v1.7.4 (2026-08-19)** — home-grid tile sits between OR Schedule/Room Assignment Audit and VNC Stocked Supply Item Search. See its own section below for architecture, matching logic, and privacy notes.
...
- Current version: **v1.7.4** (live on both dev and main)
...
- **Most recent release to main: v1.7.4 (2026-08-19), fast-forwarded** (main had zero unique commits, so this was a clean fast-forward with no merge commit — `git merge --ff-only dev` from a clean checkout, no conflicts). `main` and `dev` are now both at commit `24db9d2`, fully in sync. This brings the entire Gender Reassignment Surgery CME Approval Audit tool to production — added at v1.7.0, patched at v1.7.1 for a header-matching bug, refined at v1.7.2 (accordion structure, age filters, column reorder), simplified at v1.7.3 (Explanation column reduced to bare ICD-10 code), and expanded at v1.7.4 (accepted-approver list widened beyond CME — see its own section below) — plus the v1.7.2 Equipment Request Audit Creation User column and the v1.7.4 session-handoff-file convention. Spot-checked post-deploy: tomboonern.com serving `app.js?v=1.7.4` with HTTP 200. **Sequencing note:** earlier in this same session a bare "merge" request (no "release") was treated as ambiguous per this convention and explicitly declined; the actual release only proceeded once Tom sent the literal word "release" as a standalone message.
...
## Gender Reassignment Surgery CME Approval Audit (v1.7.0, patched/refined v1.7.1–v1.7.4)
