...
## Last updated: 2026-08-25 (v1.7.12 — live on dev, not yet on main)
...
- Current version: **v1.7.12** (live on dev only, not yet on main)
...
- **Most recent release to main: v1.7.6 (2026-08-19), fast-forwarded** (clean fast-forward, no merge commit, no conflicts each time — `git merge --ff-only dev`). `main` and `dev` are kept in sync at the same commit after every release this session. This brings the entire Gender Reassignment Surgery CME Approval Audit tool to production — added at v1.7.0, patched at v1.7.1 for a header-matching bug, refined at v1.7.2 (accordion structure, age filters, column reorder), simplified at v1.7.3 (Explanation column reduced to bare ICD-10 code), expanded at v1.7.4 (accepted-approver list widened beyond CME), and refined again at v1.7.5 (accepted-approvers reference view, Date/ICD-10 column layout fix, multi-code ICD-10 stacking) — plus the v1.7.2 Equipment Request Audit Creation User column, the v1.7.4 session-handoff-file convention, and the v1.7.6 shared column-width system spanning all five audit tables (see its own section above). Spot-checked post-deploy each time (most recently: tomboonern.com serving `app.js?v=1.7.6` with HTTP 200, confirmed via several spaced checks after brief Cloudflare edge-propagation lag — same eventual-consistency pattern seen on the v1.7.5 release, not a real issue). **Sequencing note:** earlier this session a bare "merge" request (no "release") was treated as ambiguous per this convention and explicitly declined; releases only proceed once Tom sends the literal word "release." **`dev` has since moved ahead again to v1.7.10** (2026-08-19/20: v1.7.7 campus-specific Neoprobe/TruNode display name, see Equipment Request Audit — Keywords; v1.7.8 CPT Table 1 dual-source IPO detection + Table 2 cross-reference + persistent description; v1.7.9 shared `.accordion-description` styling for both persistent-description instances; v1.7.10 clearer cross-reference wording + vertical-centering fix for that same description text, see the dedicated v1.7.8 bullet above for all three; v1.7.11 Spy ICG equipment-field alias + corrected display name, see Equipment Request Audit — Keywords; v1.7.12 Gantt visual badges — service emoji, room-designation labels, robot-platform badge, 3-light indicator, service-switching cue, see Gantt Visual Badges under Gantt Chart) — not yet released; `main` remains at v1.7.6 until Tom says "release" again. **v1.7.11 cleanup note:** the other two items requested in that pass were both already done — no `console.log` calls exist anywhere in `app.js` (including `showFromShared`/`_runAllAudits`), and `AddendumE2004.txt` was already deleted from the repo back in v1.3.91 (see Key Decisions → IPO codes) — so only the Spy ICG fix required code changes this pass.
...
**Spy ICG equipment-field alias + display name fix (v1.7.11):** the "W System SPY Imaging Stryker" device (new to Epic, never seen before this fix) was flagging as missing 100% of the time it appeared verbatim in the Equipment field, because `containsEquipmentTerm()` only matched the literal keyword "Spy ICG" against Equipment text. Added `"Spy ICG": ["spy imaging"]` to `KEYWORD_ALIASES` (`rules-data.js`), same pattern as `PTeye`→`parathyroid` — a case-insensitive substring alias checked against the Equipment field before falling back to the normal fuzzy/prefix matching. Also corrected `KEYWORD_DISPLAY_NAMES["Spy ICG"]` from `"Spy ICG"` (implicit fallback to the raw keyword) to the explicit, correctly-punctuated `"SPY (ICG) Imaging System"` (SPY capitalized per the formal system name). Verified: a case with "W System SPY Imaging Stryker" in Equipment no longer flags; a genuinely-missing case now reads "SPY (ICG) Imaging System was listed in Special Needs but not added to Equipment" in the explanation text.

**Campus-specific display name — Neoprobe/TruNode (v1.7.7):** `KEYWORD_DISPLAY_NAMES` (`rules-data.js`) maps the `Neoprobe` keyword to the display name `"TruNode"` used in explanation text — matching/detection is unaffected either way, this is display-only. **At WBDE (Davies), the display name is the literal `"Neoprobe"` instead** — Davies calls the device by its own name rather than the TruNode brand name used at the other campuses. Implemented via `KEYWORD_CAMPUS_DISPLAY_OVERRIDES` (`rules-data.js`, keyed by keyword then campus code: `{ "Neoprobe": { WBDE: "Neoprobe" } }`) and `resolveEquipmentDisplayName(keyword, campus)` (`app.js`, declared just above `auditEquipmentRows()`) — checks the campus override first, falls back to `KEYWORD_DISPLAY_NAMES`, then the raw keyword. Both places `auditEquipmentRows()` builds an explanation string (the flagged-missing branch and the all-suppressed branch) call this helper instead of doing the `KEYWORD_DISPLAY_NAMES[...] || keyword` lookup inline, reusing the `campus` value the function already derives via `deriveCampus()` rather than re-deriving it. Note `Trunode` is also its own separate, independently-matchable keyword in the list above (case-sensitive object key, unrelated to the `Neoprobe` keyword's display override) — untouched by this change. Deliberately localized as a small reusable helper + data map rather than a broader per-keyword-per-campus config system, since only this one keyword currently needs it — add another entry to `KEYWORD_CAMPUS_DISPLAY_OVERRIDES` if a similar override is needed later. Verified: a WBDE case with Neoprobe missing from Equipment shows `"Neoprobe was listed in Special Needs but not added to Equipment"`; WBVC/WBMB cases with the same missing keyword still show `"TruNode was listed..."`; a WBDE case missing a different keyword (NIM) is unaffected, confirming the override is scoped to the Neoprobe keyword only, not the whole campus.
...
### Gantt Visual Badges (v1.7.12)

Five small visual identifiers added to the Gantt, all pure display — no new detection logic, no change to rule matching/tier/suppression behavior. Data lives in `rules-data.js`; rendering lives in `buildDailyGantt()` (`app.js`).

- **`SERVICE_EMOJI`** (`rules-data.js`) — one emoji per Service value (24 entries; "Pediatrics" and "Pediatric General" are treated as the same service and both map to 🧸; "Cardiac" 🫀 and "Cardiology" ❤️‍🩹 are deliberately distinct — never share an icon, since Cardiology cases are adjacent/not-true-OR-scheduling cases that only appear on this report on hybrid-room cath lab days, per Tom). Looked up case-insensitively via `resolveServiceEmoji()` (`app.js`) against each case's `service` field; unmatched services render no emoji, no placeholder, no error.
- **`ROOM_DESIGNATED_SERVICE`** (`rules-data.js`) — room → array of designated service(s), from a recency-weighted 3-year WBVC analysis. Only strong-signal rooms are included (OR1 Gynecology — pre-existing rule, not from this analysis; OR4 Pediatric General; OR5 Ophthalmology; OR7 Cardiac; OR8 Thoracic; OR14 Vascular; OR11/OR12 both list Neurosurgery/Orthopedics/Spine since they share the pair roughly evenly). Weak/scattered-signal rooms (Plastics, General, ENT, Maxillofacial, Obstetrics, Pain Management, Radiation Oncology, Podiatry, Urology, Dental, Hand, Gastroenterology, etc.) are deliberately left unlabeled per Tom's instruction not to force-assign. Bronchoscopy's existing OR8 procedure-text-triggered room preference is unrelated to this table and unaffected — Thoracic owns OR8's service designation, Bronchoscopy is a separate mechanism.
- **`THREE_LIGHT_ROOMS`** (`rules-data.js`) — `["OR6","OR7","OR8","OR9"]`, rooms with a 3-light setup.
- **Room-key normalization**: `CAMPUS_CONFIG.WBVC.rooms`/`case.room` use "OR 7"-style strings (with a space); `ROOM_DESIGNATED_SERVICE`/`THREE_LIGHT_ROOMS` keys are the no-space "OR7" form. `roomKeyFor(room)` (`app.js`, strips whitespace) bridges the two at lookup time — the data constants themselves were kept exactly as specified rather than reformatted.
- **Robot platform badge**: reuses the existing HARD-1/HARD-2 signal, does not re-detect. `auditRoomRules()` (`app.js`) now captures `robotType` ("DV5"/"SP"/`null`) directly from the same `firedRules` array already computed per-case for rule evaluation (checking for `rule.id === "hard-1"` / `"hard-2"`) and stores it on each `cases[]` entry — zero new equipment-text matching.
- **Case block rendering** (`.gantt-block-surgeon`, in `buildDailyGantt()`): service emoji, then (if `robotType`) 🤖 + the platform code ("DV5"/"SP") in a raised, smaller `.gantt-superscript` span, then the surgeon name — in that order. Real CSS (`vertical-align: super; font-size: 0.65em`) since there's no native Unicode superscript for multi-letter text.
- **Room row label rendering**: room number → designated-service emoji(s) (deduplicated via `roomDesignationEmoji()`, e.g. OR11/OR12 show 🧠🦴🩻 once each, not repeated) → 3-light indicator (💡 + `.gantt-superscript` "3") if the room is in `THREE_LIGHT_ROOMS`. Example: "OR 7 🫀 💡³".
- **`.gantt-fixed-col` width bump (72px → 108px, `GANTT_LABEL_W` constant updated to match)**: caught during verification — the original 72px fixed room-label column clipped the widened label content (room number + up to 3 service emoji + light-bulb badge, worst case OR11/OR12's three distinct emoji). Not a pre-existing bug; a direct consequence of this feature's added content, fixed in the same pass.
- **Service-switching cue**: 🔀 renders in the visual gap between two consecutive case blocks in the same room whose `service` differs — never at the start/end of a room's row, never between same-service neighbors. Required sorting each room's cases by clamped start time before iterating (`buildDailyGantt()` previously iterated in upload-row order, not time order — harmless before since blocks are absolutely positioned by time regardless of build order, but this feature's notion of "consecutive" needs true left-to-right adjacency). The per-room case list is now pre-filtered to only clamped-visible cases (`clampS < clampE`) before both the sort and the adjacency check, so a case entirely outside the visible time axis can't be treated as a "neighbor" for a cue that would never actually be visible next to it.
- **Verified** via a synthetic 7-case fixture (CDP/Puppeteer, headless Chrome — no reusable browser-testing harness existed anywhere in the repo despite this file's many past "verified via CDP" notes referencing one; a throwaway `puppeteer-core` script was written to the session scratchpad, not committed) covering: three consecutive same-room cases (General → General → Vascular) confirming exactly one 🔀 renders, in the correct gap, and not between the two General cases; a DV5 case and an SP case confirming the robot badge + superscript render in the correct order alongside their service emoji; an OR7 case confirming the room label shows the designated Cardiac emoji + 3-light indicator together and the case block shows the Cardiac emoji with no robot badge; a case with a Service value absent from `SERVICE_EMOJI` ("Trauma Surgery") confirming a clean no-emoji fallback with no console error. Zero console/page errors across the run. A composited single screenshot of the full rendered Gantt (all 13 rooms, all 7 fixture cases) was captured and reviewed once, both before and after the `.gantt-fixed-col` width fix.
...
- CPT audit results: rendered as 2 collapsible accordion sections (all collapsed by default) in `#cptAccordion` div; accordion built entirely by `renderResults()` via `makeAccordionSection()` helper; each header shows title + count badge + caret that rotates when open. Table 1: **Inpatient-Only CPT Codes on Outpatient Cases (Medicare)** (4 cols: Date, Location, Case #, Explanation — **v1.7.8** added a per-code Table 2 cross-reference line under Explanation and a persistent description above the headers, see the dedicated v1.7.8 bullet below) — codes in explanation shown bold (plain text, no links); Medicare filter: `indexes.payer == null` (column absent) → row is eligible (backward compat); column present + empty cell → NOT flagged; column present + value contains "medicare" (case-insensitive) → flagged; optional `payer` column accepted as ["payer", "primary payer", "financial class"]. Table 2: **CPT Code Discrepancies** — consolidated bidirectional table (replaced the old Tables 2 and 3 in v1.4.19; the old Table 3 render path, `errorMessages` return field, `unrecognizedCodes`, and `codeListTd` were removed). Audit logic in `auditRows()`: `missingCodes` = on order, valid, not on case panels (presence-based/set-membership, **not** quantity-aware as of v1.6.2 — see note below); `notOnOrderCodes` = on case panels, valid, not on the order (same extraction/normalization reversed, also presence-based); `invalidCodes` = any code from either side not in `validCptCodes`, as `{ code, origin: "order" | "case" }` objects — validity check runs first so invalid codes never appear in the directional arrays; short-code errors (`extractCodes().errors`, order side) fold into `invalidCodes` with origin "order"; `KNOWN_PROBLEM_CPTS` exclusion applies to all three buckets; a row is pushed to `discrepancyRows` if any bucket is non-empty; each row carries `{ date, sortDate, location, caseNumber, orderCodes, caseCodes, missingCodes, notOnOrderCodes, invalidCodes }`. Result object is `{ totalRows, discrepancyRows, inpatientRows }` (no `missingRows`/`errorMessages`). Table 2 accordion badge is a breakdown string: "N cases: X missing, Y not on order, Z invalid" (case counts, not code counts). Table 2 cols: Date, Location, Case #, On Order Not on Case, On Case Not on Order, Invalid CPT Codes (column headers renamed v1.4.26; underlying field names `missingCodes`/`notOnOrderCodes`/`invalidCodes` and filter chip labels unchanged); Date cell reuses the equipment audit pattern (date text + `.equip-toggle-affordance` "▶ Details", arrow rotates via `.expanded` on the `.equip-row-main` row); Case # cell: bold copyable span via `makeCopyable`. Missing cell: amber marks (click-to-copy toast) + "CPT Lookup" link + "Click to report CPT not in Epic" mailto button per code. Not on Order cell: blue marks (`background:#dbeafe;color:#1e40af`) + CPT Lookup link, no report button. Invalid cell: red marks (click-to-copy, NOT AAPC-on-click) + muted origin tag "(on order)"/"(on case)" + CPT Lookup link per code, plus one label line below: "Invalid CPT - check for typo or contact ordering provider". Empty code cells render a muted "—" via `emptyCodeTd()`. Shared helpers: `AMBER_MARK_CSS`/`BLUE_MARK_CSS`/`RED_MARK_CSS`/`SMALL_ACTION_CSS` constants, `copyCodeMark(code, css, markVisited)`, `cptLookupLink(code)`. Clicking the row toggles a detail row (`buildCptDetailRow`, `.equip-detail-row`, td colSpan=6, `.equip-detail` grid) with side-by-side "CPT Codes on Order" / "CPT Codes on Case" lists (`buildCodeListValue`): missing codes render as amber marks in the order list, not-on-order codes as blue marks in the case list, invalid codes as red marks in their origin list, all other codes plain text, comma-separated; empty list renders "None"; all detail marks click-to-copy (no row-visited); header row includes right-aligned "Report an issue" `.rule-flag-btn` → mailto subject "CPT Audit Issue", body "CASE: [n]\n\nISSUE: ". Summary metric label for `missingCount` is "CPT discrepancy cases" (= `discrepancyRows.length`). Row-visited highlighting: clicking a case number adds `row-visited` inside `makeCopyable()`; marks/links/buttons in the three code cells add it in their own handlers (detail-pane marks excepted); `addRowVisitedDelegation` function is defined but no longer called from `renderResults`.

**Table 1 dual-source IPO detection + Table 2 cross-reference + persistent description (v1.7.8):** three related changes, all scoped to `auditRows()`/`renderCptTables()` in `app.js`.
1. **Dual-source detection**: `inpatientMatches` previously only checked `orderList` (Insurance Info) against `inpatientOnlyCodes`. Now checks both `orderList` and `caseList` (CPT Codes - All Panels) — a code flags Table 1 if it appears in *either* source, counted once even if it appears in both (order-side matches listed first via a `seenInpatientCodes` Set, then any case-only additions, for stable/predictable display order). This closes a real gap: an IPO code documented only on the case (not the order) was previously invisible to Table 1 entirely.
2. **Table 2 cross-reference (missing/not-on-order only, never invalid)**: for each IPO code in `inpatientMatches`, `inpatientCrossRefs` checks whether that exact code also appears in the *same case's* already-computed `missingCodes` or `notOnOrderCodes` (no separate join needed — Table 1 and Table 2 are built in the same `dataRows.forEach` iteration per case, so these arrays are already in scope before either table's row gets pushed). `invalidCodes` is deliberately **never** checked — an invalid code can't be an IPO code, since IPO membership only applies to valid, recognized CPTs (verified empirically: every code in `inpatientOnlyCodes` is expected to also be in `validCptCodes` — **with one known exception, `C9606`, present in `inpatientOnlyCodes` but absent from `validCptCodes`**, a pre-existing data gap unrelated to this feature; noted here rather than silently fixed, since editing the large hardcoded CPT sets wasn't in scope for this change — worth a follow-up data-refresh pass at some point). `missingCodes`/`notOnOrderCodes` are mutually exclusive per code by construction, so there's never ambiguity about which label applies. Render: `buildInpatientExplanationCell()` (replaces the old plain `explanationTd()` call for Table 1 only) appends one muted line per matched code below the main bold-code sentence. **Wording updated in v1.7.10** to mirror Table 2's own "on order"/"on case" framing and always name the specific code (dropping the earlier v1.7.8 "This code"/multi-code-name branching, which is no longer needed since the code is now always named): `"Please review: CPT {code} is listed on the surgical order but is missing from the case's CPT codes."` (missing) / `"Please review: CPT {code} is on the case's CPT codes but is missing from the surgical order."` (not on order).
3. **Persistent description**: a `.accordion-description`-classed `<p>` (see the shared-class note below), first child appended to Table 1's accordion body before the `table-wrap` — same visual/structural position as the CME Approval Audit's "Missing CME Approval" accordion description (above the column headers, below the accordion title). Always shown, not conditional on results. Exact text: `"CMS designates these procedures as inpatient-only. Billing or scheduling them as outpatient can result in claim denial and compliance risk. Please verify the CPT was entered correctly against the order — if correct, contact the provider to either change the case to Surgery Admit or select a non-IPO CPT."`

**`.accordion-description` shared class (v1.7.9, `styles.css`):** both this Table 1 note and the CME accordion's equivalent note (originally added v1.7.2, worded independently of this one) were duplicating the exact same inline `style.cssText` in `app.js` rather than sharing a class. v1.7.9 extracted `.accordion-description` and pointed both `note.className` assignments at it instead of the inline string — bumped from a muted, normal-weight, flush-left `<p>` to bold/near-black (`var(--ink)`, the site's standard dark text color, not `var(--muted)`) with a left inset matching the table cells' own `padding: 11px 12px` horizontal inset, so the note's text lines up with the column-header text below it rather than floating flush against the container edge. Font-family/size intentionally unchanged. Verified via computed-style + text-node `Range` measurement in both tools: `fontWeight: 700`, `color: rgb(23, 50, 77)` (`--ink`), text left-edge pixel-identical to the table's own header-cell text left-edge, in both CPT Table 1 and CME.
**Vertical spacing fix (v1.7.10):** the v1.7.9 rule used `margin: 0 0 10px 12px` — since `.accordion-body` itself has no padding at all, the note (as the body's first child) sat with zero top space between it and the accordion header's bottom border, while keeping 10px below it — a visibly top-heavy result once screenshotted. Changed to `margin: 0; padding: 12px 0 12px 12px;` — symmetric 12px top/bottom padding (plus the same 12px left inset as before, now as padding instead of margin). Verified via `getBoundingClientRect()` comparison against the header's bottom edge and the table-wrap's top edge: 14px space above the text, 15px below (the 1px difference is sub-pixel font-metrics rounding from the text-node `Range` measurement, not a real asymmetry) — confirmed via before/after screenshot crops too. Any future third instance of this "persistent accordion description" pattern should reuse `.accordion-description` rather than a new inline style.

Verified via a 5-case synthetic fixture (CDP, headless Chrome): an IPO code present only in CPT Codes - All Panels now flags Table 1 (previously missed); an IPO code present in both sources flags once, no duplicate; a case whose IPO code also shows "not on order" in Table 2 gets the cross-ref line; a case whose IPO code also shows "missing" gets the cross-ref line; a case with an *unrelated* invalid-code discrepancy for a different code shows no cross-ref line for its (unaffected) IPO code; a case with an *unrelated* missing-code discrepancy for a different code likewise shows no cross-ref line; a case with no related Table 2 discrepancy at all shows no extra line; the persistent description renders correctly above the headers. Zero console errors.
