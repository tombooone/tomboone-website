    ...
    document.getElementById("morBackHome").addEventListener("click", () => showView("home"));
    ...
    function showView(viewName) {
      ...
      document.getElementById("morView").classList.toggle("active", viewName === "mor");
  ...
  // ── MOR Automation ──────────────────────────────────────────────────────────
    ...
    const MOR_BUDGET = {
      VNC: { IP: [394,379,420,404,383,405,431,390,380,417,383,416], OP: [363,340,395,379,373,400,412,400,400,422,395,428] },
      DAV: { IP: [95,93,97,93,90,96,100,94,93,100,86,95],           OP: [298,281,326,314,303,322,338,317,312,336,318,344] },
      MBC: { IP: [107,103,113,108,103,109,116,105,103,113,102,112], OP: [160,150,175,168,164,175,182,174,172,183,172,187] }
    };
    const MOR_CAMPUS_MAP = { 10: "VNC", 13: "DAV", 30: "MBC" };
    const MOR_CAMPUSES = ["DAV", "MBC", "VNC"];
    const MOR_MONTH_NAMES = ["January","February","March","April","May","June","July","August","September","October","November","December"];
    function morMN(m) { return MOR_MONTH_NAMES[m - 1]; }

    (function() {
      let _s3File = null, _epicFile = null;
      const _s3In   = document.getElementById("morS3Input");
      const _epicIn = document.getElementById("morEpicInput");
      const _runBtn = document.getElementById("morRunBtn");
      const _status = document.getElementById("morStatus");

      function _updateBtn() { _runBtn.disabled = !(_s3File && _epicFile); }

      _s3In.addEventListener("change", () => {
        _s3File = _s3In.files[0] || null;
        _status.classList.remove("error");
        _status.textContent = "Select both files to continue.";
        _updateBtn();
      });
      _epicIn.addEventListener("change", () => {
        _epicFile = _epicIn.files[0] || null;
        _status.classList.remove("error");
        _status.textContent = "Select both files to continue.";
        _updateBtn();
      });
      ...
      _runBtn.addEventListener("click", async () => {
        ...
        _status.classList.remove("error");
        _status.textContent = "Processing...";
        try {
          await _morRunPipeline(_s3File, _epicFile, _status);
        } catch (err) {
          console.error(err);
          _status.textContent = "Error: " + (err.message || "Processing failed.");
    ...
    async function _morRunPipeline(s3File, epicFile, statusEl) {
      if (typeof XLSX === "undefined") throw new Error("Report generation library not loaded. Check your network connection.");
      statusEl.textContent = "Parsing S3 CSV...";
      const s3Rows = await _morParseS3Csv(s3File);
      if (!s3Rows.length) throw new Error("No OR department rows found in S3 CSV after filtering.");
      statusEl.textContent = "Parsing Epic XLSX...";
      const epicMap = await _morParseEpicXlsx(epicFile);
      statusEl.textContent = "Joining data...";
      for (const r of s3Rows) { const m = epicMap.get(r.logId); if (m) { r.surgeon = m.surgeon; r.service = m.service; } }
      statusEl.textContent = "Building report...";
      const dates = _morComputeDates(s3Rows);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, _morSheet1(s3Rows, dates), "Surg Vol Overall Table");
      XLSX.utils.book_append_sheet(wb, _morSheet2(s3Rows, dates), "Surg Vol Summary");
      XLSX.utils.book_append_sheet(wb, _morTop50(s3Rows, dates, null, ""), "Top 50 Surgeons");
      XLSX.utils.book_append_sheet(wb, _morTop50(s3Rows, dates, "Inpatient", "Inpatient Only"), "Top 50 Surgeons - IP");
      XLSX.utils.book_append_sheet(wb, _morTop50(s3Rows, dates, "Outpatient", "Outpatient Only"), "Top 50 Surgeons - OP");
      XLSX.utils.book_append_sheet(wb, _morSvcFocus(s3Rows, dates, "Inpatient",
        ["GENERAL","NEUROSURGERY","CARDIAC","TRANSPLANT"],
        ["ORTHOPEDICS","PLASTICS","ENT","GYNECOLOGY"], "Inpatient Only"), "Inpatient Service Focus");
      XLSX.utils.book_append_sheet(wb, _morSvcFocus(s3Rows, dates, "Outpatient",
        ["GYNECOLOGY","GENERAL","PLASTICS","UROLOGY"],
        ["OPHTHALMOLOGY","ENT","ORTHOPEDICS","NEUROSURGERY"], "Outpatient Only"), "Outpatient Service Focus");
      XLSX.utils.book_append_sheet(wb, _morSheet8(s3Rows, dates), "IP Decline vs OP Comparison");
      XLSX.utils.book_append_sheet(wb, _morAllSvc(s3Rows, dates, null, ""), "All Services - Total");
      XLSX.utils.book_append_sheet(wb, _morAllSvc(s3Rows, dates, "Inpatient", "Inpatient Only"), "All Services - IP");
      XLSX.utils.book_append_sheet(wb, _morAllSvc(s3Rows, dates, "Outpatient", "Outpatient Only"), "All Services - OP");
      statusEl.textContent = "Complete. Downloading...";
      XLSX.writeFile(wb, "CPMC_MOR_" + dates.PM_YEAR + "_" + String(dates.PM_NUM).padStart(2, "0") + ".xlsx");
    }

    async function _morParseS3Csv(file) {
      const text = (await file.text()).replace(/^﻿/, "");
      const lines = text.split(/\r?\n/).filter(l => l.trim());
      if (!lines.length) throw new Error("S3 CSV appears to be empty.");
      const rawH = parseDelimitedRow(lines[0]);
      const headers = rawH.map(h => h.trim().toLowerCase());
      const NEED = ["logid","surgerydate","surgerymonthkey","accountingunitfacilitycode","departmentname","acctbasecls_ha_c_name"];
      const idx = {};
      for (const col of NEED) {
        const i = headers.indexOf(col);
        if (i === -1) throw new Error("S3 CSV is missing required column: " + col);
        idx[col] = i;
      }
      const rows = [];
      for (let i = 1; i < lines.length; i++) {
        const cols = parseDelimitedRow(lines[i]);
        const dept = (cols[idx.departmentname] || "").trim().toUpperCase();
        if (!dept.includes("OR") || dept.includes("LD")) continue;
        const code = parseInt(cols[idx.accountingunitfacilitycode] || "0", 10);
        const sd = parseInt(cols[idx.surgerydate] || "0", 10);
        const mk = parseInt(cols[idx.surgerymonthkey] || "0", 10);
        if (!sd || !mk) continue;
        rows.push({ logId: String(cols[idx.logid] || "").trim(), surgeryDate: sd, monthKey: mk,
          campus: MOR_CAMPUS_MAP[code] || null, type: (cols[idx.acctbasecls_ha_c_name] || "").trim(),
          surgeon: null, service: null });
      }
      return rows;
    }

    const _morEpicCols = [
      { key: "caseNum",  label: "Case #",       accepted: ["case #", "case number"] },
      { key: "surgeon",  label: "Lead Surgeon",  accepted: ["lead surgeon"] },
      { key: "service",  label: "Service",       accepted: ["service"] },
    ];

    async function _morParseEpicXlsx(file) {
      const rows = await readXlsxRows(file, _morEpicCols);
      const pop = rows.filter(hasData);
      const hi = findHeaderInfoForColumns(pop, _morEpicCols);
      if (!hi) throw new Error("Epic XLSX is missing required columns (Case #, Lead Surgeon, Service).");
      const { indexes, headerRowIndex } = hi;
      const map = new Map();
      for (const row of pop.slice(headerRowIndex + 1)) {
        const cn = cell(row, indexes.caseNum).trim();
        if (!cn) continue;
        const rawS = cell(row, indexes.surgeon);
        const i = rawS.search(/,\s*md\b/i);
        const surgeon = ((i >= 0 ? rawS.slice(0, i) : rawS).trim()) || null;
        map.set(cn, { surgeon, service: cell(row, indexes.service).trim() || null });
      }
      return map;
    }

    function _morComputeDates(rows) {
      const serials = rows.map(r => r.surgeryDate).filter(d => d > 0);
      if (!serials.length) throw new Error("No valid SurgeryDate values found.");
      const MAX_DATE = new Date((Math.max(...serials) - 25569) * 86400000);
      const AY = MAX_DATE.getUTCFullYear(), AM = MAX_DATE.getUTCMonth() + 1;
      let PM_NUM = AM - 1, PM_YEAR;
      if (PM_NUM === 0) { PM_NUM = 12; PM_YEAR = AY - 1; } else { PM_YEAR = AY; }
      const PRIOR_YEAR = PM_YEAR - 1;
      const CYM = PM_YEAR * 100 + PM_NUM, PYM = PRIOR_YEAR * 100 + PM_NUM;
      const CY_YTD_START = PM_YEAR * 100 + 1, PY_YTD_START = PRIOR_YEAR * 100 + 1;
      const R12_START = PM_NUM === 12 ? PM_YEAR * 100 + 1 : (PM_YEAR - 1) * 100 + (PM_NUM + 1);
      const R12_END = CYM;
      const lastDayPM = new Date(Date.UTC(PM_YEAR, PM_NUM, 0)).getUTCDate();
      const ANALYSIS_DATE_STR = morMN(AM) + " " + MAX_DATE.getUTCDate() + ", " + AY;
      const PM_LABEL = morMN(PM_NUM) + " " + PM_YEAR;
      const PY_PM_LABEL = morMN(PM_NUM) + " " + PRIOR_YEAR;
      const r12SM = PM_NUM === 12 ? 1 : PM_NUM + 1, r12SY = PM_NUM === 12 ? PM_YEAR : PM_YEAR - 1;
      const ROLLING_LABEL = morMN(r12SM) + " " + r12SY + " - " + morMN(PM_NUM) + " " + PM_YEAR;
      return { PM_NUM, PM_YEAR, PRIOR_YEAR, CYM, PYM, CY_YTD_START, PY_YTD_START, R12_START, R12_END, lastDayPM, ANALYSIS_DATE_STR, PM_LABEL, PY_PM_LABEL, ROLLING_LABEL };
    }

    function _morCount(rows, k0, k1, campus, type) {
      return rows.filter(r => r.monthKey >= k0 && r.monthKey <= k1 && (campus == null || r.campus === campus) && (type == null || r.type === type)).length;
    }

    function _morTopSurgeons(rows, typeFilter, n, dates) {
      const { R12_START, R12_END, CYM, PYM, CY_YTD_START, PY_YTD_START } = dates;
      const matched = rows.filter(r => r.surgeon && (typeFilter == null || r.type === typeFilter));
      const map = new Map();
      for (const r of matched) {
        if (!map.has(r.surgeon)) map.set(r.surgeon, { surgeon: r.surgeon, rolling: 0, cym: 0, pym: 0, cyYtd: 0, pyYtd: 0 });
        const s = map.get(r.surgeon);
        if (r.monthKey >= R12_START && r.monthKey <= R12_END) s.rolling++;
        if (r.monthKey === CYM) s.cym++;
        if (r.monthKey === PYM) s.pym++;
        if (r.monthKey >= CY_YTD_START && r.monthKey <= CYM) s.cyYtd++;
        if (r.monthKey >= PY_YTD_START && r.monthKey <= PYM) s.pyYtd++;
      }
      return [...map.values()].sort((a, b) => b.rolling - a.rolling).slice(0, n);
    }

    function _morSheet1(rows, dates) {
      const { PM_NUM, PM_YEAR, PRIOR_YEAR, CYM, PYM, CY_YTD_START, PY_YTD_START, ANALYSIS_DATE_STR, lastDayPM, PM_LABEL, PY_PM_LABEL } = dates;
      const pmMN = morMN(PM_NUM);
      const aoa = [];
      aoa.push(["", "", "Prior Month (" + pmMN + ")", "", "", "", "", "",
        "Year-to-Date (Jan 1 - " + pmMN + " " + lastDayPM + ")¹", "", "", "", "", ""]);
      aoa.push(["", "",
        "PYM Cases (" + PY_PM_LABEL + ")", "CYM Cases (" + PM_LABEL + ")",
        "YoY Var", "Budget", "Bud Var", "Bud Var %",
        "PY YTD (" + PRIOR_YEAR + ")", "CY YTD (" + PM_YEAR + ")",
        "YoY Var", "Budget", "Bud Var", "Bud Var %"]);
      for (const [tl, ts, tk] of [["IP","Inpatient","IP"],["OP","Outpatient","OP"]]) {
        const tot = { pym: 0, cym: 0, pyYtd: 0, cyYtd: 0, pmB: 0, ytdB: 0 };
        for (const campus of MOR_CAMPUSES) {
          const pym = _morCount(rows, PYM, PYM, campus, ts);
          const cym = _morCount(rows, CYM, CYM, campus, ts);
          const pyYtd = _morCount(rows, PY_YTD_START, PYM, campus, ts);
          const cyYtd = _morCount(rows, CY_YTD_START, CYM, campus, ts);
          const pmB = MOR_BUDGET[campus][tk][PM_NUM - 1];
          const ytdB = MOR_BUDGET[campus][tk].slice(0, PM_NUM).reduce((a, b) => a + b, 0);
          aoa.push([tl, campus, pym, cym, cym - pym, pmB, cym - pmB, pmB ? (cym - pmB) / pmB : 0,
            pyYtd, cyYtd, cyYtd - pyYtd, ytdB, cyYtd - ytdB, ytdB ? (cyYtd - ytdB) / ytdB : 0]);
          tot.pym += pym; tot.cym += cym; tot.pyYtd += pyYtd; tot.cyYtd += cyYtd; tot.pmB += pmB; tot.ytdB += ytdB;
        }
        aoa.push([tl, "Total", tot.pym, tot.cym, tot.cym - tot.pym,
          tot.pmB, tot.cym - tot.pmB, tot.pmB ? (tot.cym - tot.pmB) / tot.pmB : 0,
          tot.pyYtd, tot.cyYtd, tot.cyYtd - tot.pyYtd,
          tot.ytdB, tot.cyYtd - tot.ytdB, tot.ytdB ? (tot.cyYtd - tot.ytdB) / tot.ytdB : 0]);
      }
      aoa.push(["Analysis Date: " + ANALYSIS_DATE_STR + " | Data Source: S3FinDepot  ¹YTD = Jan 1 - " + pmMN + " " + lastDayPM + ", " + PM_YEAR]);
      const ws = XLSX.utils.aoa_to_sheet(aoa);
      ws["!merges"] = [{ s: { r: 0, c: 2 }, e: { r: 0, c: 7 } }, { s: { r: 0, c: 8 }, e: { r: 0, c: 13 } }];
      return ws;
    }

    function _morSheet2(rows, dates) {
      const { PM_NUM, PM_YEAR, CYM, PYM, CY_YTD_START, PY_YTD_START, ANALYSIS_DATE_STR, lastDayPM, PM_LABEL } = dates;
      const pmMN = morMN(PM_NUM);
      const aoa = [["", "CYM Cases", "CYM-PYM Var", "CYM-PYM Var %", "CYM Budget", "CYM Bud Var", "CYM Bud Var %", "YTD Budget", "YTD Bud Var", "YTD Bud Var %"]];
      for (const [label, ts, tk] of [["IP","Inpatient","IP"],["OP","Outpatient","OP"]]) {
        const cym = MOR_CAMPUSES.reduce((s, c) => s + _morCount(rows, CYM, CYM, c, ts), 0);
        const pym = MOR_CAMPUSES.reduce((s, c) => s + _morCount(rows, PYM, PYM, c, ts), 0);
        const cyYtd = MOR_CAMPUSES.reduce((s, c) => s + _morCount(rows, CY_YTD_START, CYM, c, ts), 0);
        const cymB = MOR_CAMPUSES.reduce((s, c) => s + MOR_BUDGET[c][tk][PM_NUM - 1], 0);
        const ytdB = MOR_CAMPUSES.reduce((s, c) => s + MOR_BUDGET[c][tk].slice(0, PM_NUM).reduce((a, b) => a + b, 0), 0);
        const cymVar = cym - pym, cymVarPct = pym ? cymVar / pym : 0;
        const cymBV = cym - cymB, cymBVPct = cymB ? cymBV / cymB : 0;
        const ytdBV = cyYtd - ytdB, ytdBVPct = ytdB ? ytdBV / ytdB : 0;
        aoa.push([label, cym, cymVar, cymVarPct, cymB, cymBV, cymBVPct, ytdB, ytdBV, ytdBVPct]);
      }
      aoa.push(["Analysis Date: " + ANALYSIS_DATE_STR + " | Data Source: S3FinDepot + Epic | YTD = Jan 1 - " + pmMN + " " + lastDayPM + " | CYM = " + PM_LABEL]);
      return XLSX.utils.aoa_to_sheet(aoa);
    }

    function _morTop50(rows, dates, typeFilter, suffix) {
      const { PM_YEAR, PRIOR_YEAR, PM_LABEL, PY_PM_LABEL, ANALYSIS_DATE_STR, lastDayPM, ROLLING_LABEL } = dates;
      const pmMN = morMN(dates.PM_NUM);
      const surgs = _morTopSurgeons(rows, typeFilter, 50, dates);
      const CH = ["Rank", "Surgeon", "Rolling 12M²",
        "PYM Cases (" + PY_PM_LABEL + ")", "CYM Cases (" + PM_LABEL + ")",
        "YoY Var", "PY YTD (" + PRIOR_YEAR + ")", "CY YTD (" + PM_YEAR + ")", "YoY Var"];
      const NC = CH.length;
      const aoa = [
        ["Top 1–25", ...Array(NC - 1).fill(""), "", "Top 26–50", ...Array(NC - 1).fill("")],
        [...CH, "", ...CH]
      ];
      for (let i = 0; i < 25; i++) {
        const l = surgs[i], r = surgs[i + 25];
        const lr = l ? [i+1, l.surgeon, l.rolling, l.pym, l.cym, l.cym-l.pym, l.pyYtd, l.cyYtd, l.cyYtd-l.pyYtd] : [i+1,"","","","","","","",""];
        const rr = r ? [i+26, r.surgeon, r.rolling, r.pym, r.cym, r.cym-r.pym, r.pyYtd, r.cyYtd, r.cyYtd-r.pyYtd] : [i+26,"","","","","","","",""];
        aoa.push([...lr, "", ...rr]);
      }
      aoa.push(["Analysis Date: " + ANALYSIS_DATE_STR + " | Data Source: S3FinDepot + Epic  ¹YTD = Jan 1 - " + pmMN + " " + lastDayPM + "  ²Rolling 12M = " + ROLLING_LABEL + (suffix ? " | " + suffix : "")]);
      const ws = XLSX.utils.aoa_to_sheet(aoa);
      ws["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: NC - 1 } }, { s: { r: 0, c: NC + 1 }, e: { r: 0, c: NC * 2 } }];
      return ws;
    }

    function _morSvcFocus(rows, dates, typeStr, leftSvcs, rightSvcs, footerLabel) {
      const { R12_START, R12_END, CYM, PYM, CY_YTD_START, PY_YTD_START, ANALYSIS_DATE_STR, lastDayPM } = dates;
      const pmMN = morMN(dates.PM_NUM);
      function svcSurgs(svcUpper) {
        const matched = rows.filter(r => r.surgeon && r.service && r.service.toUpperCase() === svcUpper && r.type === typeStr);
        const map = new Map();
        for (const r of matched) {
          if (!map.has(r.surgeon)) map.set(r.surgeon, { surgeon: r.surgeon, rolling: 0, pyYtd: 0, cyYtd: 0 });
          const s = map.get(r.surgeon);
          if (r.monthKey >= R12_START && r.monthKey <= R12_END) s.rolling++;
          if (r.monthKey >= PY_YTD_START && r.monthKey <= PYM) s.pyYtd++;
          if (r.monthKey >= CY_YTD_START && r.monthKey <= CYM) s.cyYtd++;
        }
        return [...map.values()].sort((a, b) => b.rolling - a.rolling).slice(0, 5);
      }
      const aoa = [["Surgeon", "PY YTD", "CY YTD", "YoY Var", "", "Surgeon", "PY YTD", "CY YTD", "YoY Var"]];
      for (let i = 0; i < leftSvcs.length; i++) {
        const ls = svcSurgs(leftSvcs[i]), rs = svcSurgs(rightSvcs[i]);
        aoa.push([leftSvcs[i], "", "", "", "", rightSvcs[i], "", "", ""]);
        for (let j = 0; j < 5; j++) {
          const l = ls[j], r = rs[j];
          aoa.push([l?l.surgeon:"", l?l.pyYtd:"", l?l.cyYtd:"", l?l.cyYtd-l.pyYtd:"", "",
            r?r.surgeon:"", r?r.pyYtd:"", r?r.cyYtd:"", r?r.cyYtd-r.pyYtd:""]);
        }
      }
      aoa.push([footerLabel + " | Analysis Date: " + ANALYSIS_DATE_STR + " | Data Source: S3FinDepot + Epic  ¹YTD = Jan 1 - " + pmMN + " " + lastDayPM]);
      return XLSX.utils.aoa_to_sheet(aoa);
    }

    function _morSheet8(rows, dates) {
      const { CYM, PYM, CY_YTD_START, PY_YTD_START, ANALYSIS_DATE_STR, lastDayPM } = dates;
      const pmMN = morMN(dates.PM_NUM);
      const SVCS = ["Orthopedics", "General", "Plastics", "Gynecology"];
      function svcRow(svc, type) {
        const f = r => r.service && r.service.toUpperCase() === svc.toUpperCase() && (type == null || r.type === type);
        const pym = rows.filter(r => r.monthKey === PYM && f(r)).length;
        const cym = rows.filter(r => r.monthKey === CYM && f(r)).length;
        const pyYtd = rows.filter(r => r.monthKey >= PY_YTD_START && r.monthKey <= PYM && f(r)).length;
        const cyYtd = rows.filter(r => r.monthKey >= CY_YTD_START && r.monthKey <= CYM && f(r)).length;
        const momVar = cym - pym, varPct = pym ? momVar / pym : 0;
        const yoyVar = cyYtd - pyYtd, yoyVarPct = pyYtd ? yoyVar / pyYtd : 0;
        return [svc, pym, cym, momVar, varPct, pyYtd, cyYtd, yoyVar, yoyVarPct];
      }
      const aoa = [["Service","PYM Cases","CYM Cases","MoM Var","Var %","PY YTD","CY YTD","YoY Var","YoY Var %"]];
      const sections = [["INPATIENT","Inpatient"],["OUTPATIENT","Outpatient"],["TOTAL",null]];
      for (let i = 0; i < sections.length; i++) {
        const [sec, type] = sections[i];
        aoa.push([sec, "", "", "", "", "", "", "", ""]);
        for (const svc of SVCS) aoa.push(svcRow(svc, type));
        if (i < sections.length - 1) aoa.push([]);
      }
      aoa.push(["Analysis Date: " + ANALYSIS_DATE_STR + " | Data Source: S3FinDepot + Epic | YTD = Jan 1 - " + pmMN + " " + lastDayPM]);
      return XLSX.utils.aoa_to_sheet(aoa);
    }

    function _morAllSvc(rows, dates, typeFilter, prefix) {
      const { CYM, PYM, CY_YTD_START, PY_YTD_START, ANALYSIS_DATE_STR, lastDayPM } = dates;
      const pmMN = morMN(dates.PM_NUM);
      const filtered = rows.filter(r => r.service && (typeFilter == null || r.type === typeFilter));
      const services = [...new Set(filtered.map(r => r.service))].sort();
      const aoa = [["Service","PYM Cases","CYM Cases","YoY Var","YoY Var %","PY YTD","CY YTD","YoY Var","YoY Var %"]];
      for (const svc of services) {
        const f = r => r.service === svc && (typeFilter == null || r.type === typeFilter);
        const pym = rows.filter(r => r.monthKey === PYM && f(r)).length;
        const cym = rows.filter(r => r.monthKey === CYM && f(r)).length;
        const pyYtd = rows.filter(r => r.monthKey >= PY_YTD_START && r.monthKey <= PYM && f(r)).length;
        const cyYtd = rows.filter(r => r.monthKey >= CY_YTD_START && r.monthKey <= CYM && f(r)).length;
        aoa.push([svc, pym, cym, cym - pym, pym ? (cym - pym) / pym : 0, pyYtd, cyYtd, cyYtd - pyYtd, pyYtd ? (cyYtd - pyYtd) / pyYtd : 0]);
      }
      const p = prefix ? prefix + " | " : "";
      aoa.push([p + "Analysis Date: " + ANALYSIS_DATE_STR + " | Data Source: S3FinDepot  ¹YTD = Jan 1 - " + pmMN + " " + lastDayPM]);
      return XLSX.utils.aoa_to_sheet(aoa);
    }
  ...
  (function() {
    ...
    document.addEventListener("keydown", (e) => {
      const tag = document.activeElement?.tagName?.toLowerCase();
      if (tag === "input" || tag === "textarea") { tomBuf = ""; return; }
      if (wormModal.classList.contains("active")) { tomBuf = ""; return; }
      if (!/^[a-zA-Z]$/.test(e.key)) { tomBuf = ""; return; }
      tomBuf += e.key.toLowerCase();
      if (tomBuf.length > 3) tomBuf = tomBuf.slice(-3);
      if (tomBuf === "tom") { tomBuf = ""; showView("mor"); }
    });
  })();
